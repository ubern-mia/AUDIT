    ...
    with st.sidebar:
        ...
        with st.sidebar.expander("Datasets", expanded=True):
            selected_set = st.selectbox(
                ...
                options=data.set.unique(),
                index=0
            )
        with st.sidebar.expander("Subjects", expanded=True):
            # Select type of plot
            selected_subject = st.selectbox(
                label="Select a patient to explore",
                options=data[data.set == selected_set].ID.unique(),
                index=0
            )
...
def show_subject_information(data):
    ...
    with col1:
        ...
        st.dataframe(temp[temp["feature_type"] == "anatomical"].drop(columns=['feature_type']).set_index("feature"),
                     use_container_width=True)

    with col2:
        st.markdown("#### Statistical features")
        st.dataframe(temp[temp["feature_type"] == "statistical"].drop(columns=['feature_type']).set_index("feature"),
                     use_container_width=True)

    with col3:
        st.markdown("#### Texture features")
        st.dataframe(temp[temp["feature_type"] == "texture"].drop(columns=['feature_type']).set_index("feature"),
                     use_container_width=True)
